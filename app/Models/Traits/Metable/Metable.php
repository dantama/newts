...
trait Metable
{
    ...
    /**
     * Retrieve the FQCN of the class to use for MetableModel models.
     *
     * @return string
     */
    protected function getMetaClassName(): string
    {
        return isset($this->metaClass) ? $this->metaClass : MetableModel::class;
    }
    ...
    {
        return isset($this->metaTable) ? $this->metaTable : $this->getTable() . '_metas';
    }
    ...
    public function setManyMeta(array $metaDictionary): void
    {
        ...
        if (method_exists($builder, 'upsert')) {
            ...
            $builder->upsert(
                $metaModels->map(function (MetableModel $model) {
                    return $model->getAttributesForInsert();
                })->all(),
                [$this->getMetaKeyName(), 'key'],
                ['type', 'value']
            );
        } else {
     ...
     *
     * @param string $key
     * @param mixed $default Fallback value if no MetableModel is found.
     *
     * @return mixed
     */
    ...
    {
        return $this->getMetaCollection()->toBase()->map(function (MetableModel $meta) {
     ...
     * @return Meta|null
     */
    public function getMetaRecord(string $key): ?MetableModel
    {
     ...
     *
     * If an array of keys is passed instead, will restrict the query to records having one or more MetableModel with any of the keys.
     *
     * @param Builder $q
    ...
    /**
     * Create a new `MetableModel` record.
     *
     * @param string $key
     * @param mixed $value
     *
     * @return MetableModel
     */
    protected function makeMeta(string $key = '', $value = ''): MetableModel
    {
        $className = $this->getMetaClassName();
        ...
        $meta->{$this->getMetaKeyName()} = $this->getKey();
        $meta->key = $key;
        $meta->value = $value;
