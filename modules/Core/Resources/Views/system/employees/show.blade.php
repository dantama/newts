...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-4">
            ...
            <div class="card border-0">
                <div class="card-body">
                    <i class="mdi mdi-tag-outline"></i> Pekerjaan tambahan
                </div>
                @if (!is_null($employee->getMeta('additional_possition')))
                    @php($data = \Modules\Core\Models\CompanyPosition::find($employee->getMeta('additional_possition')))
                    <div class="list-group list-group-flush border-top">
                        <div class="list-group-item">
                            <div class="d-flex w-100 gap-4">
                                <div class="flex-grow-1">
                                    <div class="fw-bold">{{ $data->name }}</div>
                                    <div class="text-muted">{{ $data->department->name }}</div>
                                    <i class="mdi mdi-circle text-success" style="font-size: 11pt;"></i> &nbsp; <strong>{{ Carbon::parse($employee->getMeta('additional_range')['start_at'])->isoFormat('LL') }}</strong> s.d. <strong>{{ $employee->getMeta('additional_range')['end_at'] ? Carbon::parse($employee->getMeta('additional_range')['end_at'])->isoFormat('LL') : 'tidak ditentukan' }}</strong>
                                </div>
                                @if (!$data->trashed())
                                    <form class="d-inline form-block form-confirm" action="{{ route('hrms::employment.employees.additional.destroy', ['employee' => $employee->id]) }}" method="post"> @csrf @method('DELETE')
                ...
                @else
                    <div class="card-body border-top">
                        <a class="btn btn-soft-danger" href="{{ route('hrms::employment.employees.additional.create', ['employee' => $employee->id, 'next' => url()->full()]) }}"><i class="mdi mdi-plus"></i> Tambahkan pekerjaan tambahan</a>
                    </div>
                @endif
            </div>
        </div>
