...
@section('title', 'Ubah unit departemen ')
@section('navtitle', 'Ubah unit departemen')
                            ...
                            <div class="col-xl-8 col-xxl-6">
                                <select id="type" class="form-select @error('dept_id') is-invalid @enderror" name="dept_id">
                                    ...
                                    @forelse($depts as $key => $dept)
                                        <option value="{{ $dept->id }}" @selected($dept->id == $unit_departement->dept_id)>{{ $dept->name }}</option>
                                    @empty
                            ...
                            <div class="col-xl-8 col-xxl-6">
                                <select id="type" class="form-select @error('dept_id') is-invalid @enderror" name="dept_id">
                                    ...
                                    @forelse($parents as $key => $parent)
                                        <option value="{{ $parent->id }}" @selected($parent->id == $unit_departement->parent_id)>{{ $parent->departement->name }}</option>
                                    @empty
                                        <option value="">Tidak ada data departemen</option>
                                    @endforelse
                                </select>
