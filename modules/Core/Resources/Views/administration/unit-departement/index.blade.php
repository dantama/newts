    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card border-0">
                ...
                <div class="card-body border-top border-light">
                    <form class="form-block row g-2" action="{{ route('core::administration.unit-departements.index') }}" method="get">
                        <input name="trash" type="hidden" value="{{ request('trash') }}">
                        <div class="flex-grow-1 col-auto">
                            <input class="form-control" name="search" placeholder="Cari nama ..." value="{{ request('search') }}" />
                        </div>
                        <div class="col-auto">
                            <a class="btn btn-light" href="{{ route('core::administration.unit-departements.index', request()->only('trashed', 'closed')) }}"><i class="mdi mdi-refresh"></i> <span class="d-sm-none">Reset</span></a>
                        </div>
                        <div class="col-auto">
                ...
                <div class="d-block">
                    <div class="table-responsive">
                        <table class="table-hover mb-0 table align-middle">
                            ...
                            <tbody>
                                @forelse($unitDepts as $unit)
                                    <tr @if ($unit->trashed()) class="table-light text-muted" @endif>
                                        <td>{{ $loop->iteration + $unitDepts->firstItem() - 1 }}</td>
                                        <td>
                                        ...
                                        <td class="py-2 text-end" nowrap>
                                            @if (!$unit->trashed())
                                                ...
                                                <a class="btn btn-soft-warning rounded px-2 py-1" href="{{ route('core::administration.unit-departements.create', ['unit' => $unit->id, 'next' => url()->current()]) }}" data-bs-toggle="tooltip" title="Tambah komponen"><i class="mdi mdi-plus-circle-outline"></i></a>
                                            @endif
                                        </td>
                                    </tr>
                                    <tr>
                                                ...
                                                <table class="table-borderless table-hover table-sm mb-0 table align-middle">
                                                    <thead>
                                                        <tr class="text-muted small bg-light">
                                                            ...
                                                            <th class="border-bottom fw-normal">Departemen</th>
                                                            <th class="border-bottom fw-normal">Status</th>
                                                            <th class="border-bottom fw-normal"></th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        @forelse($unit->unit_departements as $key => $dept)
                                                            <tr class="small">
                                                                <td></td>
                                                                ...
                                                                <td class="text-end">
                                                                    <div class="me-1">
                                                                        @if ($dept->trashed())
                                                                            @can('restore', $dept)
                                                                                <form class="form-block form-confirm d-inline" action="{{ route('core::administration.unit-departements.restore', ['unit' => $unit->id, 'unit_departement' => $dept->id, 'next' => url()->current()]) }}" method="post"> @csrf @method('put')
                                                                                    <button class="btn btn-soft-info rounded px-2 py-1" data-bs-toggle="tooltip" title="Pulihkan"><i class="mdi mdi-refresh"></i></button>
                                                                                </form>
                                                                            @endcan
                                                                        @else
                                                                            @can('update', $dept)
                                                                                <a class="btn btn-sm btn-soft-warning rounded px-2 py-1" href="{{ route('core::administration.unit-departements.show', ['unit' => $unit->id, 'unit_departement' => $dept->id, 'next' => url()->current()]) }}" method="post" data-bs-toggle="tooltip" title="Ubah"><i class="mdi mdi-pencil-outline"></i></a>
                                                                            @endcan
                                                                            @can('kill', $dept)
                                                                                <form class="form-block form-confirm d-inline" action="{{ route('core::administration.unit-departements.destroy', ['unit' => $unit->id, 'unit_departement' => $dept->id, 'next' => url()->current()]) }}" method="post"> @csrf @method('delete')
        ...
        <div class="col-md-4">
            ...
            <div class="card border-0">
                ...
                <div class="card-body">
                    <form action="{{ route('core::administration.unit-departements.index') }}" method="get">
                        <div class="mb-3">
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <a class="btn btn-light" href="{{ route('core::administration.unit-departements.index') }}"><i class="mdi mdi-refresh"></i> Reset</a>
                        </div>
                    </form>
                </div>
            </div>
            <div class="card border-0">
                ...
                <div class="list-group list-group-flush border-top border-light">
                    <a class="list-group-item list-group-item-action text-danger" href="{{ route('core::administration.unit-departements.index', ['trash' => !request('trash')]) }}"><i class="mdi mdi-trash-can-outline"></i> Lihat unit departemen yang {{ request('trash') ? 'tidak' : '' }} dihapus</a>
                </div>
            </div>
        </div>
    </div>
