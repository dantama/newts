...
@section('navtitle', 'Ubah Artikel')

@php($back = request('next', route('blog::article.posts.index')))

@section('content')
    <form class="form-block" action="{{ route('blog::article.posts.update', ['post' => $post->id, 'next' => $back]) }}" method="POST" enctype="multipart/form-data"> @csrf @method('PUT')
        <div class="row justify-content-center">
            <div class="col-xl-8 col-lg-8 col-sm-12">
                <div class="card border-0">
                    <div class="card-body border-bottom">
                        <div><i class="mdi mdi-plus"></i> Ubah artikel</div>
                    </div>
                    <div class="card-body">
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <div>
                                <textarea class="form-control tiny @error('content') is-invalid @enderror" name="content" rows="20">{{ old('content', $post->content) }}</textarea>
                                @error('content')
                                    <small class="text-danger d-block"> {{ $message }} </small>
                                @enderror
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div>
                                <textarea class="form-control @error('meta[keyword]') is-invalid @enderror" name="meta[keyword]" rows="1">{{ old('meta[keyword]', $post->getMeta('keyword')) }}</textarea>
                                @error('meta[keyword]')
                                    <small class="text-danger d-block"> {{ $message }} </small>
                                @enderror
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div>
                                <textarea class="form-control @error('meta[title]') is-invalid @enderror" name="meta[title]" rows="2">{{ old('meta[title]', $post->getMeta('title')) }}</textarea>
                                @error('meta[title]')
                                    <small class="text-danger d-block"> {{ $message }} </small>
                                @enderror
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div>
                                <textarea class="form-control @error('meta[description]') is-invalid @enderror" name="meta[description]" rows="4">{{ old('meta[description]', $post->getMeta('description')) }}</textarea>
                                ...
                                <div class="card card-body @error('ctg_id') is-invalid @enderror mb-0 p-2">
                                    @foreach ($categories as $category)
                                        <div class="form-check">
                                            <input type="checkbox" id="type-{{ $category->id }}" class="form-check-input" name="ctg_id[]" value="{{ $category->id }}" @checked(in_array($category->id, request('ctg_id', $post->categories->pluck('id')->toArray()))) />
                                            <label class="form-check-label" for="type-{{ $category->id }}">{{ $category->title }}</label>
                                        </div>
                                    @endforeach
                                </div>
                                @error('ctg_id')
                        ...
                        <div class="mb-3">
                            ...
                            <div class="col-lg-12">
                                <select name="tags[]" id="tags" class="form-select" multiple>
                                    @foreach ($tags as $tag)
                                        <option value="{{ $tag }}" @selected(in_array($tag, $post->tags->pluck('name')->toArray()))>{{ $tag }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
