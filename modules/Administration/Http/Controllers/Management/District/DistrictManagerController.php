...
class DistrictManagerController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $mgmt = $managements->where('id', $request->get('mgmt', ($managements->first()->id ?? 0)))->first();

        $managers = Manager::with('user', 'position')
                    ->when($trashed, function ($query) {
                        return $query->onlyTrashed();
                    })
                    ->whereManagementDistrictIn($request->get('mgmt', $managements->first()->id ?? null))
                    ->get();

        $managers_count = Manager::inDistrict()->count();    


        return view('administration::managements.districts-managers.index', compact('managements', 'mgmt', 'managers','positions','members','managers_count'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $manager = new manager([
            ...
            'managerable_type' => 'App\Models\ManagementDistrict',
            'managerable_id' => $request->input('managerable_id')
        ]);

        $manager->save();

        return redirect($request->get('next', route('administration::managements.districts-managers.index')))->with('success', 'berhasil');
    }
    ...
    public function show($manager)
    {
        ...
        return view('administration::managements.districts-managers.edit', compact('managers','positions')); 
    }
