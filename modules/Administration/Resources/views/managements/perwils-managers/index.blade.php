...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h4 class="font-weight-bold mb-0 text-gray-800">Pengurus Perwil</h4>
</div>
@endsection
...
<div class="row">
    <div class="col-md-8"> 
        <div class="card border-0 shadow mb-4">
            ...
            @endif
            <div class="card-body">
                <form class="form-block" action="{{ route('administration::managements.perwil-managers.index') }}" method="GET">
                        ...
                        <tr>
                            ...
                            <td nowrap>
                                @if($manager->isnot(auth()->user()))
                                    @if($manager->trashed())
                                        <form class="d-inline form-block form-confirm" action="{{ route('administration::managements.perwil-managers.restore', ['perwil_manager' => $manager->id]) }}" method="POST"> @csrf @method('PUT')
                                            <button class="btn btn-primary btn-sm" data-toggle="tooltip" title="Pulihkan"><i class="fa fa-sync"></i></button>
                                        </form>
                                        <form class="d-inline form-block form-confirm" action="{{ route('administration::managements.perwil-managers.kill', ['perwil_manager' => $manager->id]) }}" method="POST"> @csrf @method('DELETE')
                                            <button class="btn btn-danger btn-sm" data-toggle="tooltip" title="Hapus permanen"><i class="fa fa-trash"></i></button>
                                        </form>
                                    @else
                                        @can('admin')
                                            <form class="d-inline form-block form-confirm" action="{{ route('administration::managements.perwil-managers.adminable', ['perwil_manager' => $manager->id]) }}" method="POST"> @csrf @method('PUT')
                                        ...
                                        @endcan
                                        <a class="btn btn-warning btn-sm" data-toggle="tooltip" title="Ubah" href="{{ route('administration::managements.perwil-managers.show', ['perwil_manager' => $manager->id]) }}"><i class="fa fa-edit"></i></a>
                                        <form class="d-inline form-block form-confirm" action="{{ route('administration::managements.perwil-managers.destroy', ['perwil_manager' => $manager->id]) }}" method="POST"> @csrf @method('DELETE')
        ...
        <div class="card border-0 shadow mb-4">
            ...
            <div class="card-body pb-1">
                <form class="form-row" method="POST" action="{{ route('administration::managements.perwil-managers.store',['next' => request('next',route('administration::managements.perwil-managers.index'))]) }}">@csrf
    ...
    <div class="col-md-4">
        <div class="card border-left-danger border-0 shadow py-2 mb-4">
            <div class="card-body">
                <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                        <div class="text-xs font-weight-bold text-danger text-uppercase mb-1">Total pengurus perwil</div>
        ...
        <div class="card border-0 shadow mb-4">
            ...
            <div class="list-group list-group-flush">
                <a class="list-group-item list-group-item-action text-danger" href="{{ route('administration::managements.perwil-managers.index', ['trash' => request('trash', 0) ? null : 1, 'pimwil' => request('pimwil')]) }}"><i class="fa fa-trash"></i> Tampilkan data {{ request('trash', 0) ? 'tidak dihapus' : 'dihapus' }}</a>
            </div>
        </div>
    </div>
</div>
