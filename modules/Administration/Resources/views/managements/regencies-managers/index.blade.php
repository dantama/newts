...
@section('title', 'Pengurus Daerah')

@section('content-header')
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h4 class="font-weight-bold mb-0 text-gray-800">Pengurus daerah</h4>
</div>
@endsection
...
<div class="row">
    <div class="col-md-8">
        <div class="card border-0 shadow mb-4">
            ...
            @endif
            <div class="card-body">
                <form class="form-block" action="{{ route('administration::managements.regencies-managers.index') }}" method="GET">
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0 border-bottom">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td nowrap>
                                @if($manager->isnot(auth()->user()))
                                    @if($manager->trashed())
                                        <form class="d-inline form-block form-confirm" action="{{ route('administration::managements.regencies-managers.restore', ['manager' => $manager->id]) }}" method="POST"> @csrf @method('PUT')
                                            <button class="btn btn-primary btn-sm" data-toggle="tooltip" title="Pulihkan"><i class="fa fa-sync"></i></button>
                                        </form>
                                        <form class="d-inline form-block form-confirm" action="{{ route('administration::managements.regencies-managers.kill', ['manager' => $manager->id]) }}" method="POST"> @csrf @method('DELETE')
                                            <button class="btn btn-danger btn-sm" data-toggle="tooltip" title="Hapus permanen"><i class="fa fa-trash"></i></button>
                                        </form>
                                    @else
                                        @can('admin')
                                            <form class="d-inline form-block form-confirm" action="{{ route('administration::managements.regencies-managers.adminable', ['manager' => $manager->id]) }}" method="POST"> @csrf @method('PUT')
                                        ...
                                        @endcan
                                        <a class="btn btn-warning btn-sm" data-toggle="tooltip" title="Ubah" href="{{ route('administration::managements.regencies-managers.show', ['manager' => $manager->id]) }}"><i class="fa fa-edit"></i></a>
                                        <form class="d-inline form-block form-confirm" action="{{ route('administration::managements.regencies-managers.destroy', ['manager' => $manager->id]) }}" method="POST"> @csrf @method('DELETE')
                        ...
                        @empty
                        <tr>
                            <td colspan="7">Tidak ada data pengurus</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="card border-0 shadow mb-4">
            ...
            <div class="card-body pb-1">
                <form class="form-row" method="POST" action="{{ route('administration::managements.regencies-managers.store',['next' => request('next',route('administration::managements.regencies-managers.index'))]) }}">@csrf
    ...
    <div class="col-md-4">
        ...
        @include('administration::managements.regencies.includes.regency-detail', ['regency' => $mgmt])
        <div class="card border-0 shadow mb-4">
            <div class="card-header">Lanjutan</div>
            <div class="list-group list-group-flush">
                <a class="list-group-item list-group-item-action text-danger" href="{{ route('administration::managements.regencies-managers.index', ['trash' => request('trash', 0) ? null : 1, 'pimwil' => request('pimwil')]) }}"><i class="fa fa-trash"></i> Tampilkan data {{ request('trash', 0) ? 'tidak dihapus' : 'dihapus' }}</a>
            </div>
        </div>
    </div>
</div>
