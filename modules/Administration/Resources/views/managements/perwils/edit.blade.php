...
<div class="row justify-content-center">
	<div class="col-md-8">
		<h4 class="font-weight-bold mb-4">
			<a class="text-decoration-none small" href="{{ route('administration::managements.perwil.index') }}"> <i class="fa fa-arrow-circle-left"></i></a> Daftar Perwil
		</h4>
		<div class="card border-0 shadow mb-4">
			<div class="card-body m-3">
				<form class="form-block" action="{{ route('administration::managements.perwil.update',['perwil' =>$perwil['id']]) }}" method="POST"> @csrf @method('PUT')
					<div class="form-group">
                        <label for="inputAddress">Nama</label>
                        <input type="text" class="form-control" id="name" name="name" value="{{ $perwil['name'] }}">
                    </div>
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <label for="inputEmail4">Email</label>
                            <input type="email" class="form-control" id="email" name="email" value="{{ $perwil['email'] ?? '' }}">
                        </div>
                        <div class="form-group col-md-6">
                            <label for="telp">No Telp</label>
                            <input type="text" class="form-control" id="telp" name="phone" value="{{ $perwil['phone'] ?? '' }}">
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group col-md-8">
                            <label for="inputAddress">Alamat</label>
                            <input type="text" class="form-control" id="alamat" name="address" value="{{ $perwil['address'] ?? '' }}">
                        </div>
                        <div class="form-group col-md-4">
                            <label for="telp">Kode Wilayah</label>
                            <input type="text" class="form-control" id="code" name="code" value="{{ $perwil['perwil_code'] ?? '' }}" {{ (auth()->user()->isManagerCenters()) ? '' : 'readonly' }}>
                        </div>
                    </div>
                    <div class="form-group">
