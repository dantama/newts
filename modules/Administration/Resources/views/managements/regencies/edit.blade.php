...
<div class="row justify-content-center">
	<div class="col-md-8">
		<h4 class="font-weight-bold mb-4">
			<a class="text-decoration-none small" href="{{ route('administration::managements.regencies.index') }}"> <i class="fa fa-arrow-circle-left"></i></a> Daftar Pimda
		</h4>
		<div class="card border-0 shadow mb-4">
			<div class="card-body m-3">
				<form class="form-block" action="{{ route('administration::managements.regencies.update',['id' =>$regency['id']]) }}" method="POST"> @csrf @method('PUT')
					<div class="form-group">
                        <label for="inputAddress">Nama</label>
                        <input type="text" class="form-control" id="name" name="name" value="{{ $regency['name'] }}">
                    </div>
                    <div class="form-group">
                        ...
                        <select name="regency_id" id="regency_id" class="form-control selectpicker" data-live-search="true">
                            ...
                            <option value="{{ $val->id }}" {{ $val->id == $regency['regency_id'] ? 'selected' : '' }}>{{ $val->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            ...
                            <input type="email" class="form-control" id="email" name="email" value="{{ $regency['email'] ?? 'belum diisi' }}">
                        </div>
                        <div class="form-group col-md-6">
                            <label for="telp">No Telp</label>
                            <input type="text" class="form-control" id="telp" name="phone" value="{{ $regency['phone'] ?? 'belum diisi' }}">
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group col-md-8">
                            <label for="inputAddress">Alamat</label>
                            <input type="text" class="form-control" id="alamat" name="address" value="{{ $regency['address'] ?? 'belum diisi' }}">
                        </div>
                        <div class="form-group col-md-4">
                            <label for="telp">Kode Daerah</label>
                            <input type="text" class="form-control" id="code" name="code" value="{{ str_pad($regency['pimda_code'], 3, '0', STR_PAD_LEFT) ?? 'belum diisi' }}" {{ (auth()->user()->isManagerCenters()) ? '' : 'readonly' }}>
                        </div>
                    </div>
                    <div class="form-group">
