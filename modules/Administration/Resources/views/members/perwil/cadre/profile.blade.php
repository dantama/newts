...
<div class="row justify-content-center">
	<div class="col-md-8">
		<h4 class="font-weight-bold mb-4">
			<a class="text-decoration-none small" href="{{ route('administration::members.perwil-cadres.show',['perwil_cadre' => $cadres['id']]) }}"> <i class="fa fa-arrow-circle-left"></i></a> Cadre Profile
		</h4>
		<div class="card border-0 shadow mb-4">
			<div class="card-body m-3">
				<form class="form-block" action="{{ route('administration::members.perwil-cadres.profile-update',['perwil_cadre' => $cadres['user']['id']], ['next' => request('next', route('administration::members.perwil-cadres.index'))]) }}" method="POST"> @csrf @method('PUT')
					<div class="form-group">
						<label for="inputAddress">Full Name</label>
						<input type="text" name="name" class="form-control col-md-12 @error('name') is-invalid @enderror" id="fullname" placeholder="Nama Lengkap" value="{{ $cadres['user']['profile']['name'] }}" required>
						@error('name')
					...
					<div class="form-group">
						...
						<input type="text" name="nik" class="form-control col-md-12" id="nik" placeholder="NIK" value="{{ $cadres['user']['profile']['nik'] }}" required>
					</div>
					<div class="form-row">
						<div class="form-group col-md-4">
							...
							<select id="sex" name="sex" class="form-control @error('sex') is-invalid @enderror" required>
								...
								<option value="0" {{ $cadres['user']['profile']['sex'] == 0 ? 'selected' : '' }}>Male</option>
								<option value="1" {{ $cadres['user']['profile']['sex'] == 1 ? 'selected' : '' }}>Female</option>
							</select>
						</div>
						<div class="form-group col-md-4">
							<label for="inputEmail4">Date of birth</label>
							<input type="date" class="form-control" id="dob" name="dob" value="{{ $cadres['user']['profile']['dob'] }}" required>
						</div>
						<div class="form-group col-md-4">
							<label for="inputEmail4">Place of birth</label>
							<input type="text" class="form-control" id="pob" name="pob" value="{{ $cadres['user']['profile']['pob'] }}" required>
						</div>
					</div>
					<div class="form-group">
						<label for="inputAddress">Prefix</label>
						<input type="text" name="prefix" class="form-control col-md-12" value="{{ $cadres['user']['profile']['prefix'] }}" id="prefix">
					</div>
					<div class="form-group">
						<label for="inputAddress">Suffix</label>
						<input type="text" name="suffix" class="form-control col-md-12" value="{{ $cadres['user']['profile']['suffix'] }}" id="suffix">
					</div>
					<div class="form-group">
