    ...
    <div class="row justify-content-center">
        <div class="col-xxl-8 col-xl-10">
            <div class="d-flex align-items-center mb-4">
                <a class="text-decoration-none" href="{{ request('next', route('administration::managers.index')) }}"><i class="mdi mdi-arrow-left-circle-outline mdi-36px"></i></a>
                <div class="ms-4">
            ...
            <div class="card mb-4 border-0">
                <div class="card-body">
                    <form class="form-block" action="{{ route('administration::managers.update', ['manager' => $manager->id, 'next' => request('next')]) }}" method="POST"> @csrf
                        ...
                        <div class="row required mb-4">
                            ...
                            <div class="col-xl-8 col-xxl-8">
                                <div class="input-group form-calculate mb-2">
                                    <input type="date" class="form-control @error('start_at') is-invalid @enderror" name="start_at" value="{{ $manager->start_at ? $manager->start_at->format('Y-m-d') : now()->format('Y-m-d') }}">
                                    <div class="input-group-text">s.d.</div>
                                    <input type="date" class="form-control @error('end_at') is-invalid @enderror" name="end_at" value="{{ $manager->end_at ? $manager->end_at->format('Y-m-d') : now()->format('Y-m-d') }}">
                                </div>
                            </div>
                            @error('start_at')
                                <small class="text-danger d-block"> {{ $message }} </small>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-lg-8">
                                <input type="text" name="contract_number" class="form-control @error('contract_number') is-invalid @enderror" value="{{ old('contract_number', $manager->contract->kd) }}">
                        ...
                        <div class="row mb-3">
                            <div class="col-lg-8 offset-lg-4 offset-xl-3">
                                ...
                                <a class="btn btn-ghost-light text-dark" href="{{ request('next', route('administration::managers.index')) }}"><i class="mdi mdi-arrow-left"></i> Kembali</a>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    <script>
        ...
        const listDept = () => {
            [].slice.call(document.querySelectorAll('[name="unit_id"] option:checked')).map((select) => {
                let c = '';
                let dept = @json($manager->unit_dept_id);
                if (select.dataset.depts) {
                    let depts = JSON.parse(select.dataset.depts);
                    for (i in depts) {
                        c += '<option value="' + i + '" ' + ((dept == i) ? ' selected' : '') + '>' + depts[i] + '</option>';
                    }
                }
        ...
        const fetchMembers = async (unitId) => {
            ...
            let member = @json($manager->member_id);
            const response = await fetch(`${url}?q=${encodeURIComponent(unitId)}`, {
                ...
                for (i in members) {
                    opt += '<option value="' + i + '" ' + ((member == i) ? ' selected' : '') + '>' + members[i] + '</option>';
                }
        ...
        const fetchUnit = async (unitId) => {
            ...
            let position = @json($manager->contract->unit_position_id);
            const response = await fetch(`${url}?q=${encodeURIComponent(unitId)}`, {
                method: 'GET',
                headers: {
                    "Authorization": `Bearer ${token}`,
                ...
                for (i in positions) {
                    opt += '<option value="' + i + '" ' + ((position == i) ? ' selected' : '') + '>' + positions[i] + '</option>';
                }
        ...
        const renderUnitFirst = () => {
            const unit = @json($manager->unit_departement->unit_id);
            const selectElement = document.querySelector('[name="unit_id"]');
            const optionToSelect = [...selectElement.options].find((option) => option.value == unit);
            if (optionToSelect) {
