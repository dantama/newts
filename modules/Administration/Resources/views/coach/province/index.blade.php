...
<div class="row">
    <div class="col-md-8"> 
        <div class="card border-0 shadow mb-4">
            ...
            @endif
            <div class="card-body">
                <form action="{{ route('administration::technical.province-coachs.index') }}" method="GET">
                    ...
                    <tbody>
                        ...
                        <tr @if($coach->trashed()) class="text-muted bg-light" @endif>
                            ...
                            <td>
                                ...
                                @else
                                    <a href="{{ route('administration::technical.province-coachs.show', ['coach' => $coach->id]) }}">{{ $coach['user']['profile']['display_name'] }}</a>
                                @endif
                            </td>                     
                            <td>{{ $coach['user']['address']['branch'] ?? '-' }}</td>                                                           
                            <td>@if($coach->user->isnot(auth()->user()))
                                    @if($coach->trashed())
                                        <form class="d-inline form-block form-confirm" action="{{ route('administration::technical.province-coachs.restore', ['coach' => $coach->id]) }}" method="POST"> @csrf @method('PUT')
                                            <button class="btn btn-primary btn-sm" data-toggle="tooltip" title="Pulihkan"><i class="fa fa-sync"></i></button>
                                        </form>
                                        <form class="d-inline form-block form-confirm" action="{{ route('administration::technical.province-coachs.kill', ['coach' => $coach->id]) }}" method="POST"> @csrf @method('DELETE')
                                            <button class="btn btn-danger btn-sm" data-toggle="tooltip" title="Hapus permanen"><i class="fa fa-delete"></i></button>
                                        </form>
                                    @else
                                        <a class="btn btn-warning btn-sm" data-toggle="tooltip" title="Ubah siswa" href="{{ route('administration::technical.province-coachs.show', ['coach' => $coach->id]) }}"><i class="fa fa-edit"></i></a>
                                        <form class="d-inline form-block form-confirm" action="{{ route('administration::technical.province-coachs.destroy', ['coach' => $coach->id]) }}" method="POST"> @csrf @method('DELETE')
    ...
    <div class="col-md-4">
        <div class="card border-0 shadow mb-4">
            ...
            <div class="card-body">
                <span class="h1 mb-0 font-weight-bold">{{ $coachProvince_count }}</span>
        ...
        <div class="card border-0 shadow mb-4">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('administration::technical.province-coachs.store',['next' => request('next',route('administration::technical.province-coachs.index'))]) }}">@csrf
                  <div class="form-group">
        ...
        <div class="card border-0 shadow mb-4">
            ...
            <div class="list-group list-group-flush">
                <a class="list-group-item list-group-item-action text-danger" href="{{ route('administration::technical.province-coachs.index', ['trash' => request('trash', 0) ? null : 1]) }}"><i class="fa fa-trash"></i> Tampilkan Pelatih dihapus</a>
            </div>
        </div>
    </div>
</div>
